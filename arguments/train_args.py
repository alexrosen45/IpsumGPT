...
def get_args():
    ...
    parser.add_argument('--data_dir', type=str, default='datasets/ipsum', help='Dataset directory (ex. --out_dir="datasets/ipsum")')
...
def arg_checks(args):
    ...
    if os.path.exists('models/' + args.out_dir) and not args.override_dir and not args.resume:
        raise FileExistsError(f"Directory {'models/' + args.out_dir} exists and overriding is not permitted. Use --override_dir to override.")
    # Resuming from non-existent directory
    if args.resume and not os.path.exists('models/' + args.out_dir):
        raise FileExistsError(f"Cannot resume from training directory {'models' + args.out_dir} that does not exist. Try specifying\
